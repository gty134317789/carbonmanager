...
export default {
  ...
  created() {
    let _this=this
    axios.get('http://localhost:8181/company/findById/'+this.$route.query.id).then(function (resp){
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios.put('http://localhost:8181/company/update',_this.ruleForm).then(function (resp) {
